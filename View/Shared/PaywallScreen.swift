...
struct PaywallScreen: View {
    ...
    private func purchase() async {
        ...
        do {
            ...
            if let selected = planOptions.first(where: { $0.id == selectedPlanId })?.package {
                ...
                package = prioritizePackages(offering.availablePackages).first ?? offering.availablePackages.first!
            }

            let (_, customerInfo, userCancelled) = try await Purchases.shared.purchase(package: package)
            guard !userCancelled else { return }

            let active = subscriptionManager.hasActiveEntitlement(in: customerInfo)
    ...
    private func loadOfferings() async {
        do {
            ...
            #if DEBUG
            print("🔍 [PaywallScreen] loadOfferings error: \(error)")
            #endif
        }
    }
